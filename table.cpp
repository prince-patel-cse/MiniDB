...
Table::Table(const std::string &name, const std::vector<Column> &cols)
{
    this->cols = cols;
    this->name = name;
    // Set up our columns layout maps so we know where columns are and what types they hold.
    for (size_t i = 0; i < cols.size(); ++i)
    {
        s[cols[i].name] = cols[i].type;
        colIndex[cols[i].name] = i;
        // Make a search index helper for each column!
        indexes.emplace_back(cols[i].name + "_idx", name, cols[i].name, i, cols[i].type);
    }
    // Also make a search index helper for the row ID!
...
{
    // If the input row doesn't have the same number of columns as our table schema, it's bad!
...
void Table::insertRow(const std::vector<std::string> &row)
{
    ...
    if (validate(row))
    {
        ...
        std::cout << "Row added successfully" << std::endl
                  << "ID : " << count + 1 << std::endl;
        ids[++count] = rows.size() - 1; // Map this row's logical ID to its memory slot.

        // Add this new row's values to all our search index guides!
        for (size_t i = 0; i < cols.size(); ++i)
        {
            indexes[i].insert(row[i], { 0, 0, count });
        }
        indexes[cols.size()].insert(std::to_string(count), { 0, 0, count });
...
{
    // Print all columns of a row formatted nicely: "colName : value , colName2 : value2"
    for (size_t i = 0; i < cols.size(); ++i)
    {
        std::cout << cols[i].name << " : " << row[i] << (i + 1 < cols.size() ? " , " : "");
    }
    std::cout << std::endl;
}

void Table::printRow(const std::vector<std::string> &row, const std::vector<std::string> &colsToPrint) const
{
    ...
    for (size_t i = 0; i < colsToPrint.size(); ++i)
    {
        ...
        if (colIndex.count(colName))
        {
            ...
            std::cout << colName << " : " << row[idx] << (i + 1 < colsToPrint.size() ? " , " : "");
        }
    }
    std::cout << std::endl;
}

void Table::print(const std::vector<std::vector<std::string>> &rowsToPrint) const
{
...
void Table::updateRow(int id, const std::string &key, const std::string &val)
{
    ...
    int pIdx = ids.at(id);
    std::string oldVal = rows[pIdx][idx]; // Remember the old value before we overwrite it.
    ...
    // Update our search index guides: remove the old value and add the new value!
    indexes[idx].remove(oldVal, id);
    indexes[idx].insert(val, { 0, 0, id });
...
void Table::deleteRow(int id)
{
    ...
    else
    {
        ...
        // Remove this row's values from all our search index guides!
        for (size_t i = 0; i < cols.size(); ++i)
        {
            indexes[i].remove(rowValues[i], id);
        }
        indexes[cols.size()].remove(std::to_string(id), id);
...
void Table::rollback(int n)
{
    ...
    // Undo changes backwards until we reach step N and overwrite our active state!
    Table t = *this;
    for (int i = static_cast<int>(versions.size()) - 1; i > n; i--)
        execute(t, versions[i]);
    *this = t;
    ...
    // Rebuild all our search indexes from scratch so they are clean and match the rolled-back state!
    ...
    for (const auto &pair : ids)
    {
        ...
        for (size_t i = 0; i < cols.size(); ++i)
        {
            indexes[i].insert(row[i], { 0, 0, logicalId });
        }
        indexes[cols.size()].insert(std::to_string(logicalId), { 0, 0, logicalId });
    }
...
static void serializeVersions(Pager &pager, const std::vector<Version>& versions, uint32_t &versionHeadPageId)
{
    if (versions.empty())
    {
        versionHeadPageId = 0;
    ...
    for (const auto &v : versions)
    {
        ...
        // If the current version log entry does not fit on the current page, write this page to disk 
        // and link a brand new page!
...
static void deserializeVersions(Pager &pager, uint32_t versionHeadPageId, std::vector<Version>& versions)
{
    versions.clear();
    if (versionHeadPageId == 0) return;
...
void Table::serializeToPages(Pager &pager)
{
    ...
    else
    {
        ...
        for (const auto &pair : ids)
        {
            ...
            // If the row doesn't fit on this page, write it to disk and link a new page!
            ...
            // Record this row's final page ID and offset so we can update the index guides!
            finalLocations[logicalId] = { currPageId, static_cast<uint32_t>(offset), logicalId };
        ...
        // Update all our search index guides with the correct page IDs and offsets we just wrote!
        for (auto &pair : finalLocations)
        {
            ...
            for (size_t colIdx = 0; colIdx < cols.size(); ++colIdx)
            {
                auto &idx = indexes[colIdx];
                if (idx.memIndex.count(row[colIdx]))
                {
                    auto &locs = idx.memIndex[row[colIdx]];
                    for (auto &l : locs)
                    {
...
void Table::deserializeFromPages(Pager &pager, uint32_t headPage, uint32_t indexRootPage)
{
    ...
    if (headPageId != 0)
    {
        ...
        while (currPageId != 0)
        {
            ...
            for (uint16_t i = 0; i < num_rows; ++i)
            {
                ...
                {
                    rows.push_back(row);
                    ids[logicalId] = rows.size() - 1; // Map the loaded row ID to its memory slot.
                    if (logicalId > count)
                    {
                        count = logicalId;
                    }
